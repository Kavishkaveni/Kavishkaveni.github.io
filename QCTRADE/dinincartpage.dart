...
class _DineInOrderCartPageState extends State<DineInOrderCartPage> {
  ...
  cart.forEach((name, qty) {
    ...
    items.add({
      "product_id": product["product_id"],   // string like "PROD2002"
      "quantity": qty,
      "unit_price": (product["price"] as num).toInt(),
  ...
  final newOrder = await QcTradeApi.createOrder(payload);
  ...
  await QcTradeApi.sendToKitchen(orderId);

  Navigator.pop(context); // close loading

  // OPEN order tracking with REAL backend data
  Navigator.pushReplacement(
    context,
    MaterialPageRoute(
      builder: (_) => OrderTrackingPage(orderData: newOrder),
    ),
  );
},
                  child: Text(
