...
export default function Devices() {
  ...
  const handleConnect = async (protocol) => {
    ...
    try {
      ...
      // 2) create session (protocol = SSH/RDP/HTTPS)
      const sessionData = {
        device_id: selectedDevice.id,
        protocol: protocol, // SSH / RDP / HTTPS  (CJ treats HTTPS as WEB)
      ...
      // ✅ Option-A for Web: download the SAME .rdp (CJ/CH handle Chrome on Jump Host)
      if (protocol === 'HTTPS') {
        console.log('Web (HTTPS) Option-A: downloading .rdp for:', selectedDevice.name)

        // Use a dedicated helper that hits GET /sessions/:uuid/rdp-web
        if (typeof ApiService.downloadRdpFileWeb === 'function') {
          await ApiService.downloadRdpFileWeb(sessionResponse.uuid)
        } else {
          // Fallback if helper not yet added: reuse generic downloader with explicit path
