...
export default function Devices() {
    ...
    const getStatusClass = () => {
      if (type === "protocol") {
        const protocolColors = {
          ...
          WEB: "bg-success bg-opacity-10 text-success",
        }
  ...
  const submitAddDevice = async (e) => {
    e.preventDefault()
    try {
      await ApiService.createDevice(formData)  // includes web_url
  ...
  const submitEditDevice = async (e) => {
    e.preventDefault()
    try {
      await ApiService.updateDevice(selectedDevice.id, formData) // includes web_url
      await loadDevices(searchTerm)
  ...
  const handleConnect = async (protocol) => {
    ...
    try {
      ...
      // keep MULTISSH as its own protocol
const backendProtocol =
  protocol === 'WEB'
    ...
    : protocol
      ...
      if (protocol === 'RDP') {
        ...
        setShowConnectModal(false); setSelectedDevice(null); return
      }
      if (protocol === 'SSH') {
        if (typeof ApiService.downloadRdpFileSsh === 'function') {
          await ApiService.downloadRdpFileSsh(sessionResponse.uuid)
        } else {
    // fallback to direct fetch
    const path = `/sessions/${sessionResponse.uuid}/rdp-ssh`
    const url = `${window.location.origin.replace(/\/$/, '')}${path.startsWith('/api') ? '' : '/api'}${path}`
    const resp = await fetch(url)
    if (!resp.ok) throw new Error(`HTTP error! status: ${resp.status}`)
    const blob = await resp.blob()
    const u = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = u; a.download = `session-${sessionResponse.uuid}.rdp`
  ...
  return
      ...
      if (protocol === 'WEB') {
        if (typeof ApiService.downloadRdpFileWeb === 'function') {
          await ApiService.downloadRdpFileWeb(sessionResponse.uuid)
        } else if (typeof ApiService.downloadFileFromPath === 'function') {
          await ApiService.downloadFileFromPath(`/sessions/${sessionResponse.uuid}/rdp-web`, `session-${sessionResponse.uuid}.rdp`, 'application/rdp')
        } else {
          const path = `/sessions/${sessionResponse.uuid}/rdp-web`
          const url = `${window.location.origin.replace(/\/$/, '')}${path.startsWith('/api') ? '' : '/api'}${path}`
          const resp = await fetch(url)
          if (!resp.ok) throw new Error(`HTTP error! status: ${resp.status}`)
          const blob = await resp.blob()
     ...
     //  NEW: Handle Multiple SSH Connection (same logic as SSH/Web)
      if (protocol === 'MULTISSH') {
        if (typeof ApiService.downloadRdpFileSsh === 'function') {
          await ApiService.downloadRdpFileSsh(sessionResponse.uuid)
        } else {
    // fallback to direct fetch
    const path = `/sessions/${sessionResponse.uuid}/rdp-mulitissh`
    const url = `${window.location.origin.replace(/\/$/, '')}${path.startsWith('/api') ? '' : '/api'}${path}`
    const resp = await fetch(url)
    if (!resp.ok) throw new Error(`HTTP error! status: ${resp.status}`)
    const blob = await resp.blob()
  ...
  return
      ...
      setError(`Failed to initiate ${protocol} connection: ${err.message || err}`)
      console.error('Error creating session / downloading file:', err)
    }
  }
  ...
  const Modal = ({ show, onClose, title, children }) => {
    ...
    return (
      <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center"
        ...
        <div className="bg-white rounded-3 shadow-lg" style={{ width: "90%", maxWidth: "500px", maxHeight: "90vh", overflowY: "auto" }}>
    ...
    <div className="w-100">
      ...
      {error && (
        <div className="alert alert-danger d-flex align-items-center mb-4" role="alert">
          ...
          <div>
            {error}
            <button className="btn btn-link btn-sm p-0 ms-2 text-decoration-none" onClick={() => loadDevices(searchTerm)}>
              Try again
            </button>
          </div>
        </div>
        ...
        <div className="table-responsive">
          {loading ? (
            ...
            <div className="text-center py-5 text-muted">
              ...
              <p>{searchTerm ? `No devices match "${searchTerm}"` : "Start by adding your first device"}</p>
            </div>
          ) : (
              ...
              <tbody>
                {filteredDevices.map((device) => (
                  <tr key={device.id}>
                    ...
                    <td>
                      {device.web_url ? (
                        <a href={device.web_url} target="_blank" rel="noopener noreferrer">
                          {device.web_url}
                        </a>
                      ) : (
                    ...
                    <td>
                      <button className="btn btn-link btn-sm p-1 text-muted" title="Connect" onClick={() => handleConnectDevice(device)}>🔗</button>
                      <button className="btn btn-link btn-sm p-1 text-muted" title="Edit" onClick={() => handleEditDevice(device)}>✏️</button>
                      <button className="btn btn-link btn-sm p-1 text-muted" title="Info">ℹ️</button>
                      <button className="btn btn-link btn-sm p-1 text-muted" title="Delete" onClick={() => handleDeleteDevice(device)}>🗑️</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <form onSubmit={submitAddDevice}>
          <div className="p-4">
            <div className="mb-3">
              ...
              <input type="text" className="form-control" value={formData.name} autoFocus
                     onChange={(e) => setFormData((prev) => ({ ...prev, name: e.target.value }))} required />
            </div>

            <div className="mb-3">
              <label className="form-label">IP Address</label>
              <input type="text" className="form-control" value={formData.ip}
                     onChange={(e) => setFormData((prev) => ({ ...prev, ip: e.target.value }))} required />
            </div>
            ...
            <div className="mb-3">
              ...
              <select className="form-select" value={formData.access}
                      onChange={(e) => setFormData({ ...formData, access: e.target.value })} required>
                <option value="">Select Protocol</option>
                <option value="SSH">SSH</option>
                <option value="RDP">RDP</option>
                <option value="WEB">WEB</option>
                <option value="MULTISSH">MULTISSH</option>
              </select>
            </div>

            {/* NEW: Web URL field */}
            <div className="mb-3">
              <label className="form-label">Web URL (for WEB)</label>
              <input
                type="url"
                className="form-control"
                placeholder="https://your-app.example.com:9443"
                value={formData.web_url}
                onChange={(e) => setFormData({ ...formData, web_url: e.target.value })}
              />
              {formData.access === 'WEB' && !formData.web_url && (
                <small className="text-danger">Required when Access Protocol is WEB</small>
              )}
            </div>
        ...
        <form onSubmit={submitEditDevice}>
          <div className="p-4">
            ...
            <div className="mb-3">
              <label className="form-label">IP Address</label>
              <input type="text" className="form-control" value={formData.ip}
                     onChange={(e) => setFormData({ ...formData, ip: e.target.value })} required />
            </div>
            <div className="mb-3">
              <label className="form-label">Device Type</label>
              <select className="form-select" value={formData.type}
        ...
        <Modal show={showConnectModal} onClose={closeModal} title={`Connect to ${selectedDevice.name}`}>
          <div className="p-4">
            ...
            <div className="d-grid gap-3">
              <button className="btn btn-outline-primary d-flex align-items-center justify-content-start gap-3 p-3"
              ...
              <button className="btn btn-outline-primary d-flex align-items-center justify-content-start gap-3 p-3"
                ...
                <div className="text-start">
                  ...
                  <small className="text-muted">Download .rdp → CJ/CH auto-login</small>
                </div>
              </button>
              ...
              {/* NEW: Multiple SSH Connection */}
<button
  className="btn btn-outline-primary d-flex align-items-center justify-content-start gap-3 p-3"
  onClick={() => handleConnect("MULTISSH")}
>
  <span style={{ fontSize: "1.5rem" }}>🧩</span>
  <div className="text-start">
    <div className="fw-medium">Multiple SSH Connection</div>
    <small className="text-muted">Download .rdp for Multiple SSH Client</small>
  </div>
</button>
            </div>
          </div>
      ...
      {showDeleteModal && selectedDevice && (
        <Modal show={showDeleteModal} onClose={closeModal} title="Delete Device">
          ...
          <div className="d-flex justify-content-end gap-2 p-4 border-top">
            <button type="button" className="btn btn-secondary" onClick={closeModal}>Cancel</button>
            <button type="button" className="btn btn-danger" onClick={confirmDelete}>Delete Device</button>
          </div>
        </Modal>
      )}
    </div>
