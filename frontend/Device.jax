...
export default function Devices() {
  ...
  const handleConnect = async (protocol) => {
    ...
    try {
      ...
      // 2) create session
      // IMPORTANT: for Web flow we send protocol "WEB" to the backend
      const backendProtocol = protocol === 'WEB' ? 'WEB' : protocol // SSH/RDP/WEB
      const sessionData = {
      ...
      // 3) protocol branches
      if (protocol === 'RDP') {
        await ApiService.downloadRdpFile(sessionResponse.uuid) // /sessions/:uuid/rdp
      ...
      if (protocol === 'WEB') {
        // EXACTLY like RDP: download the RDP file for Web flow
        // (backend route must exist: GET /sessions/:uuid/rdp-web)
        if (typeof ApiService.downloadRdpFileWeb === 'function') {
          ...
          if (typeof ApiService.downloadFileFromPath === 'function') {
            await ApiService.downloadFileFromPath(path, `session-${sessionResponse.uuid}.rdp`, 'application/rdp')
          } else {
            // last-resort: do a simple fetch
            ...
            a.href = u
            a.download = `session-${sessionResponse.uuid}.rdp`
        ...
        <Modal show={showConnectModal} onClose={closeModal} title={`Connect to ${selectedDevice.name}`}>
          <div className="p-4">
            ...
            <div className="d-grid gap-3">
              ...
              {/* WEB connection uses RDP download flow (Option A) */}
              <button
                className="btn btn-outline-primary d-flex align-items-center justify-content-start gap-3 p-3"
