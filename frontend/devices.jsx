...
export default function Devices() {
  ...
  const submitAddDevice = async (e) => {
    ...
    try {
      ...
      await loadDevices(searchTerm) // Reload devices
  ...
  const handleConnect = async (protocol) => {
    ...
    try {
      ...
      // First, get vault entries for this device to find credentials
      ...
      // Use the first available credential (in a real app, user might choose)
      const credentials = vaultEntries[0]

      // Create a new session in the backend
      const sessionData = {
        device_id: selectedDevice.id,
        protocol: protocol, // Keep original case (SSH/RDP)
        username: credentials.username,
        user_identity: "admin", // This should come from authenticated user context
      }
      ...
      if (protocol === 'RDP') {
        ...
        console.log('Downloading RDP file for session:', sessionResponse.uuid)
        ...
        console.log(`RDP file downloaded for ${selectedDevice.name}`)
      } else if (protocol === 'SSH') {
        // For SSH, we might also generate a connection file or provide instructions
