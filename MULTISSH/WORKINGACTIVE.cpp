...
#include <winhttp.h>
#include <vector>
#include <thread>   // <-- needed for the background "ended" status updater
					...
					if (CreateProcessW(nullptr, &cmd[0], nullptr, nullptr, FALSE, 0, nullptr, nullptr, &si, &pi))
					{
                       ...
                       // ------------------------------------------------------------
						...
						std::thread([hProcCopy, hThreadCopy, thisUser, thisHost, hwnd]() {
							// Wait for this specific PuTTY session to close
							WaitForSingleObject(hProcCopy, INFINITE);

							CloseHandle(hThreadCopy);
							CloseHandle(hProcCopy);

							// Mark that exact session as ended
							...
							// Refresh the UI after marking "Ended"
