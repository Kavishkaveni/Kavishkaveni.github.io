	...
	case WM_COMMAND:
		if (HIWORD(wParam) == BN_CLICKED)
		{
			...
			case 201: // OK clicked
			{
				...
				std::wstring logMsg = L"[PAM LOGIN] Username=" + std::wstring(username);
				logEvent(logMsg);
				...
				int result = ValidatePAMCredentials(userStr, passStr);
					...
					if (result == 200)
					{
						MessageBox(hwnd, L"Authentication successful.", L"Success", MB_OK | MB_ICONINFORMATION);
						logEvent(L"[PAM LOGIN] Authentication successful");

						// ---- Paste from here ----
						// Prepare backend URL and JSON body
						std::wstring postUrl = L"/api/sessions/" + g_sessionUuid + L"/device/" + std::to_wstring(selectedDeviceId) + L"/authenticate";
						...
						{
							logEvent(L"[DEBUG] Raw backend response: " + s2ws(response));
							...
							logEvent(L"[DEBUG] SSH Command: " + cmd);
						...
						{
							MessageBoxW(hwnd, L"Failed to fetch SSH credentials from backend.", L"Error", MB_OK);
						}
						// ---- End of paste ----
				...
				{
					MessageBox(hwnd, L"Invalid PAM credentials. Please try again.", L"Authentication Failed", MB_OK | MB_ICONERROR);
					logEvent(L"[PAM LOGIN] Invalid credentials");
				}
				else
				{
					MessageBox(hwnd, L"Server error. Please contact admin.", L"Error", MB_OK | MB_ICONERROR);
					logEvent(L"[PAM LOGIN] Server error occurred");
				}
				return 0;
			}
			case 202: // Cancel
