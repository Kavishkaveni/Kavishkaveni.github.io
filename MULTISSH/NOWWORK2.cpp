...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
			...
			UINT_PTR, DWORD_PTR refData) -> LRESULT
		{
			...
			case WM_PAINT:
			{
				...
				{
					SetBkMode(hdc, TRANSPARENT);
					SetTextColor(hdc, RGB(150, 150, 150)); // grey placeholder
	...
	case WM_COMMAND:
	{
		...
		case 3: // Search button clicked (task/change)
		{
			...
			// Simple mapping for TSK/CHG numbers
...
// ------------------------------------------------------------
		...
		ExcludeClipRect(hdc, 30, 170, 240, 195);
	   ...
	   // ------------------------------------------------------------
		...
		if (g_sessions.empty()) {
			...
			for (const auto& s : g_sessions) {
				...
				int statusX = rcText.left + 650; // <-- fixed column position
	...
	case WM_CTLCOLOREDIT:
	{
		...
		{
			SetBkColor(hdcEdit, RGB(245, 247, 250)); // sidebar bg
			SetTextColor(hdcEdit, RGB(10, 10, 10));  // dark text visible on gray
			static HBRUSH hbrBg = CreateSolidBrush(RGB(245, 247, 250));
			return (INT_PTR)hbrBg;
		}

		if (hwndEdit == hSearchBox)
		{
			SetBkColor(hdcEdit, RGB(255, 255, 255)); // top white
			...
			case 201: // OK clicked
			{
				...
				if (http_post_json(L"192.168.8.199", 9000, postUrl, jsonBody, &status, response))
				{
					...
					if (CreateProcessW(nullptr, &cmd[0], nullptr, nullptr, FALSE, 0, nullptr, nullptr, &si, &pi))
					{
						...
						SessionInfo s;
						s.device = L"Device-" + std::to_wstring(selectedDeviceId);
						s.user = sshUser;
						s.ip = sshHost;
						s.active = true;
					   ...
					   // ------------------------------------------------------------
						...
						std::thread([hProcCopy, hThreadCopy, thisUser, thisHost, hwnd]() {
							...
							if (updated) {
								...
								if (parent) {
									SendMessage(parent, WM_PAINT, 0, 0);
									RedrawWindow(parent, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
								}
							}
