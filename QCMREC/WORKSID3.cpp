...
static void RunCaptureLoop(std::atomic<bool>& running)
{
	...
	if (FAILED(hr)) { LogRec(L"DuplicateOutput failed hr=0x%08X", hr); CoUninitialize(); return; }
	...
	while (running)
	{
		...
		if (!ok) {
			LogRec(L"WTSQuerySessionInformation failed ec=%lu — stopping capture", GetLastError());
			running = false;
			break;
		}
		...
		if (state != WTSActive) {
			LogRec(L"Session %s state=%d — stopping capture",
