// qcmrec_dxgi.cpp
// Minimal DXGI Desktop Duplication recorder (MP4) for manual or service mode.
// Manual:   QCMREC.exe start <UUID> <SESSIONID>
// Service:  QCMREC.exe   (listens on TCP 10444 for "start <UUID> <SESSIONID>")
...
static void RunCaptureLoop(std::atomic<bool>& running)
{
    ...
    MessageBoxW(nullptr,
        L"Recording started.\nIt will stop automatically when CJ disconnects (or ENTER in manual mode).",
        L"QCMREC", MB_OK | MB_ICONINFORMATION);
    ...
    while (running)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            for (UINT y = 0; y < height; ++y)
                memcpy(dst + y * pitch, src + (height - 1 - y) * map.RowPitch, pitch);
...
int RunServiceMode()
{
    ...
    addr.sin_port = htons(10444); addr.sin_addr.s_addr = INADDR_ANY;
    bind(listenSock, (sockaddr*)&addr, sizeof(addr));
    listen(listenSock, 1);
    wprintf(L"[QCMREC] Listening on port 10444...\n");
    ...
    if (len > 0) {
        ...
        if (_wcsicmp(cmd.c_str(), L"start") == 0) {
            ...
            std::thread stopThread([&] {
                char tmp[4];
                while (recv(client, tmp, sizeof(tmp), 0) > 0) {}
                running = false;
            });
...
int wmain(int argc, wchar_t** argv)
{
    ...
    else
    {
        MessageBoxW(nullptr,
            ...
            L"Or run with no arguments to act as service (listens on 10444).",
            L"QCMREC", MB_OK | MB_ICONINFORMATION);
        return 0;
    }
}
