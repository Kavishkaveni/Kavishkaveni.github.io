// qcmrec_dxgi.cpp
// DXGI Desktop recorder with both manual mode and Windows Service mode.
...
void ReportSvcStatus(DWORD state){
    ...
    SetServiceStatus(gSvcStatusHandle,state==SERVICE_RUNNING?SERVICE_RUNNING:state);
}

void WINAPI SvcCtrlHandler(DWORD ctrl){
    if(ctrl==SERVICE_CONTROL_STOP){
        ReportSvcStatus(SERVICE_STOP_PENDING);
        // We rely on TCP disconnect to stop capture loop
...
int wmain(int argc, wchar_t** argv)
{
    if (argc>=4 && _wcsicmp(argv[1],L"start")==0){
        ...
        SERVICE_TABLE_ENTRY DispatchTable[]={{(LPWSTR)L"QCMREC",(LPSERVICE_MAIN_FUNCTION)SvcMain},{NULL,NULL}};
