...
static void RunCaptureInSession(DWORD targetSession, const std::wstring& uuid)
{
	...
	// after: if (!DuplicateTokenEx(...)) { ... return; }
	DWORD sid = targetSession;
	if (!SetTokenInformation(hPrimary, TokenSessionId, &sid, sizeof(sid))) {
		LogRec(L"SetTokenInformation(TokenSessionId) failed ec=%lu", GetLastError());
	}
	...
	wchar_t cmd[512];
	StringCchPrintfW(cmd, 512, L"\"%s\" start %s %u --silent", exe, uuid.c_str(), targetSession);

	STARTUPINFOW si{};
	si.cb = sizeof(si);
	si.lpDesktop = (LPWSTR)L"winsta0\\default";   // attach to interactive desktop

	PROCESS_INFORMATION pi{};
	DWORD flags = CREATE_UNICODE_ENVIRONMENT; // no NEW_CONSOLE for silent service launch
	// <-- NEW_CONSOLE keeps an interactive desktop/GPU context without showing a message box
...
int wmain(int argc, wchar_t** argv)
{
	...
	if (argc >= 4 && _wcsicmp(argv[1], L"start") == 0) {
		...
		bool silent = (argc >= 5 && _wcsicmp(argv[4], L"--silent") == 0);
