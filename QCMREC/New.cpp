...
static void BuildFramePath(wchar_t* out, size_t cch, int frameIndex)
{
    ...
    StringCchPrintfW(out, cch,
        L"C:\\REC\\capture_%04u%02u%02u_%02u%02u%02u_frame%04d.png",
...
int wmain(int argc, wchar_t** argv)
{
    if (argc < 2 || _wcsicmp(argv[1], L"start") != 0) {
        MessageBoxW(nullptr,
            L"Usage:\n\nQCMREC.exe start\n\n"
            L"- Shows a popup\n- Captures desktop frames via DXGI\n"
            L"- Saves PNGs to C:\\REC\n- Press ENTER in console to stop.",
    ...
    while (running) {
        ...
        // Create staging tex (once) with correct size/format
        if (!staging) {
            ...
            // approximate stride (will get actual from Map)
            pitch = width * 4;
        }
        ...
        if (SUCCEEDED(hr)) {
            ...
            BuildFramePath(path, MAX_PATH, ++frameIndex);
            SavePNG(wic.Get(), path, width, height, map.RowPitch, (BYTE*)map.pData);
    ...
    MessageBoxW(nullptr, L"Recording stopped. Check C:\\REC for PNG frames.",
                L"QCMREC (DXGI)", MB_OK | MB_ICONINFORMATION);
    return 0;
}
