...
{
    DWORD sid = *(DWORD*)p; delete (DWORD*)p;
    while(g_ShouldPoll){
        LPTSTR ps=nullptr; DWORD cb=0; WTS_CONNECTSTATE_CLASS st=WTSDisconnected;
        if(WTSQuerySessionInformationW(WTS_CURRENT_SERVER_HANDLE,sid,WTSConnectState,&ps,&cb)&&ps){ st=*(WTS_CONNECTSTATE_CLASS*)ps; WTSFreeMemory(ps);}
        if(st!=WTSActive){ wchar_t m[128]; StringCchPrintfW(m,128,L"Session %lu not active (%d). Stop.",sid,(int)st); LogLine(m); StopRecorder(); break;}
        Sleep(1500);
    }
    return 0;
}
...
void ShowPopup(DWORD sid){
    ...
    WTSSendMessageW(WTS_CURRENT_SERVER_HANDLE,sid,(LPWSTR)t,(DWORD)(wcslen(t)*sizeof(wchar_t)),(LPWSTR)b,(DWORD)(wcslen(b)*sizeof(wchar_t)),MB_OK|MB_ICONINFORMATION,0,&resp,FALSE);
}
...
DWORD FindRdpSession(){
    ...
    if(WTSEnumerateSessionsW(WTS_CURRENT_SERVER_HANDLE,0,1,&info,&cnt)){
        for(DWORD i=0;i<cnt;i++) if(info[i].State==WTSActive){
            ...
            if(WTSQuerySessionInformationW(WTS_CURRENT_SERVER_HANDLE,info[i].SessionId,WTSClientProtocolType,&pT,&cb)&&pT){
                if(*(USHORT*)pT==2){res=info[i].SessionId; WTSFreeMemory(pT); break;} WTSFreeMemory(pT);
            }
        }
        WTSFreeMemory(info);
    }
...
BOOL LaunchRecorder(DWORD sid){
    ...
    HANDLE hPrim=nullptr; if(!DuplicateTokenEx(hUser,TOKEN_ALL_ACCESS,nullptr,SecurityImpersonation,TokenPrimary,&hPrim)){CloseHandle(hUser); LogLine(L"DuplicateTokenEx failed"); return FALSE;} CloseHandle(hUser);
    ...
    EnsureRecDir();
    SYSTEMTIME st;GetLocalTime(&st);wchar_t out[MAX_PATH];
    ...
    const wchar_t* grab=L" -f gdigrab -i desktop "; // or " -f ddagrab -i desktop "
    wchar_t cmd[1024];StringCchPrintfW(cmd,1024,L"\"%s\" -y%ls-framerate 15 -c:v libx264 -preset ultrafast \"%s\"",FFMPEG_PATH,grab,out);
    STARTUPINFOW si{};si.cb=sizeof(si);si.lpDesktop=(LPWSTR)L"winsta0\\default";PROCESS_INFORMATION piP{};
    if(!CreateProcessAsUserW(hPrim,nullptr,cmd,nullptr,nullptr,FALSE,CREATE_NO_WINDOW|CREATE_UNICODE_ENVIRONMENT,env,nullptr,&si,&piP)){LogLine(L"CreateProcessAsUserW failed");if(env)DestroyEnvironmentBlock(env);CloseHandle(hPrim);return FALSE;}
    g_FFmpegProc=piP;CloseHandle(piP.hThread);if(env)DestroyEnvironmentBlock(env);CloseHandle(hPrim);
    ShowPopup(sid);
    g_SessionId=sid;g_ShouldPoll=TRUE;DWORD* s=new DWORD(sid);g_PollThread=CreateThread(nullptr,0,PollThread,s,0,nullptr);
...
DWORD WINAPI CtrlHandlerEx(DWORD c,DWORD e,LPVOID d,LPVOID){
    if(c==SERVICE_CONTROL_STOP){StopRecorder();g_ServiceStatus.dwCurrentState=SERVICE_STOPPED;SetServiceStatus(g_StatusHandle,&g_ServiceStatus);LogLine(L"Service stopped");return NO_ERROR;}
    if(c==SERVICE_CONTROL_SESSIONCHANGE){
        ...
        else if(e==WTS_SESSION_LOGOFF||e==WTS_SESSION_DISCONNECT)StopRecorder();
        return NO_ERROR;
    }
...
void WINAPI ServiceMain(DWORD,LPWSTR*){
    ...
    if(GetFileAttributesW(MANUAL_TRIGGER)!=INVALID_FILE_ATTRIBUTES){DWORD sid=WTSGetActiveConsoleSessionId();LaunchRecorder(sid);}
