...
static void DoConnect(const std::wstring& uuid, const std::wstring& backendHost, INTERNET_PORT backendPort)
{
	...
	if (_wcsicmp(proto.c_str(), L"WEB") == 0) {
		...
		const wchar_t*  chHost = L"localhost";      // use L"127.0.0.1" or a real IP if CH is remote
		...
		DWORD httpStatus = 0;
		bool ok = http_post_json(chHost, chPort, L"/", json, &httpStatus);
		if (!ok) LogF(L"CH POST failed (status=%lu). Is CH running on %u?", httpStatus, (unsigned)chPort);
		else     LogF(L"CH accepted WEB request (status=%lu).", httpStatus);
	...
	// Launch mstsc in user's session
	wchar_t args[256];
	...
	// Give mstsc time to read the credential
	Sleep(25000);

	// Delete credential in user's session
